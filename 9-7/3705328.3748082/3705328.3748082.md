...
Each domain has its own set of users \( \left( {\mathcal{U}}^{S}\right. \) and \( {\mathcal{U}}^{T} \) ), items \( \left( {\mathcal{I}}^{S}\right. \) and \( {\mathcal{I}}^{T} \) ), and interaction records \( \left( {\mathcal{R}}^{S}\right. \) and \( \left. {\mathcal{R}}^{T}\right) \) . Given the observed data from both \( {\mathcal{D}}^{T} \) and \( {\mathcal{D}}^{S} \) , CDR methods initially employ an embedding layer to derive embedding tables \( {\mathbf{E}}_{u}^{T} \) and \( {\mathbf{E}}_{u}^{S} \) . Specifically, for a user \( u \in  {\mathcal{U}}^{o} \) , there exist user embeddings \( {\mathbf{e}}^{S} \) in the source domain and \( {\mathbf{e}}^{T} \) in the target domain, while other users possess embeddings solely within their respective domains.

The parameter \( \Theta \) training objective is to enhance recommendation performance in the target domain through techniques such as transfer learning [51, 68], formulated as:
...
\[
{UGF} = {\left| \frac{1}{\left| {\mathcal{U}}^{o}\right| }\mathop{\sum }\limits_{{u \in  {\mathcal{U}}^{o}}}\mathcal{M}\left( {L}_{u}\right)  - \frac{1}{\left| {\mathcal{U}}^{T} \smallsetminus  {\mathcal{U}}^{o}\right| }\mathop{\sum }\limits_{{u \in  {\mathcal{U}}^{T} \smallsetminus  {\mathcal{U}}^{o}}}\mathcal{M}\left( {L}_{u}\right) \right| }_{s}
\]
...
\[
\left\{  {{u}_{1},{u}_{2},\ldots ,{u}_{N}}\right\}   = \underset{u \in  {\mathcal{U}}^{o}}{\arg \operatorname{top}N}\operatorname{Sim}\left( {{\mathbf{e}}_{\text{ non }}^{T},{\mathbf{e}}_{u}^{T}}\right) , \tag{10}
\]
...
where \( {\mathbf{e}}_{\text{ non }}^{T} \) denotes the embedding of the non-overlapping user \( {u}_{\text{ non }} \) in the target domain, \( \operatorname{Sim}\left( {\cdot , \cdot  }\right) \) computes the behavior similarity between different users, and Aggr(·) represents an aggregation function (e.g., mean). After identifying the top- \( N \) overlapping users, their embeddings in the source domain are aggregated to construct the virtual user embedding as the representation in the source domain for non-overlapping target-domain user \( {u}_{\text{ non }} \) .
...
\[
{\beta }_{u}^{\text{ user }} = \frac{\left( {{\mathbf{W}}_{q}^{\text{ user }}{\mathbf{e}}_{non}^{T} + {\mathbf{b}}_{q}^{\text{ user }}}\right) {\left( {\mathbf{W}}_{k}^{\text{ user }}{\mathbf{e}}_{u}^{T} + {\mathbf{b}}_{k}^{\text{ user }}\right) }^{\top }}{\sqrt{d}}\text{ , } \tag{15}
\]
...
\[
{\beta }_{u}^{\text{ item }} = \frac{\left( {{\mathbf{W}}_{q}^{\text{ item }}{\mathbf{g}}_{non}^{T} + {\mathbf{b}}_{q}^{\text{ item }}}\right) {\left( {\mathbf{W}}_{k}^{\text{ item }}{\mathbf{g}}_{u}^{T} + {\mathbf{b}}_{k}^{\text{ item }}\right) }^{\top }}{\sqrt{d}}. \tag{18}
\]
...
\[
{\mathcal{L}}_{\text{ constrain }} = \log \mathbb{E}\left\lbrack  {e}^{-2{\begin{Vmatrix}{\mathrm{e}}_{u}^{{S}^{\prime }} - {\mathrm{e}}_{{u}^{\prime }}^{{S}^{\prime }}\end{Vmatrix}}^{2}}\right\rbrack  ,\;u,{u}^{\prime } \in  {\mathcal{U}}^{T} \smallsetminus  {\mathcal{U}}^{o} \tag{22}
\]
...
\[
\mathop{\min }\limits_{{\Theta  \smallsetminus  {\Theta }_{\text{ gen }}}}{\mathcal{L}}_{\mathrm{{CDR}}}\text{ , } \tag{23}
\]

\[
\mathop{\min }\limits_{{\Theta }_{\text{ gen }}}{\gamma }_{2}{\mathcal{L}}_{\text{ super }} + \left( {1 - {\gamma }_{2}}\right) {\mathcal{L}}_{\text{ constrain }}\text{ , } \tag{24}
\]
...
We analyze the effectiveness of different components via the following variants: (1) VUG without \( {\mathcal{L}}_{\text{ constrain }} \) (w/o \( {\mathcal{L}}_{\text{ constrain }} \) ),(2) VUG without \( {\mathcal{L}}_{\text{ super }}\left( \right. \) w/o \( {\mathcal{L}}_{\text{ super }} \) ),(3) VUG without user-user attention (w/o \( {\alpha }_{u}^{\text{ user }} \) ), and (4) VUG without item-item attention (w/o \( {\alpha }_{u}^{\text{ item }} \) ). The results are presented in Table 3. They suggest that all components positively contribute to model performance and fairness.
...
![7_152_239_1494_547_0.jpg](images/7_152_239_1494_547_0.jpg)
...
![7_933_1549_712_354_0.jpg](images/7_933_1549_712_354_0.jpg)
...
[39] Zihan Wang, Yonghui Yang, Le Wu, Richang Hong, and Meng Wang. 2025. Making Non-Overlapping Matters: An Unsupervised Alignment Enhanced Cross-Domai Cold-Start Recommendation. TKDE 37 (2025), 2001-2014.
