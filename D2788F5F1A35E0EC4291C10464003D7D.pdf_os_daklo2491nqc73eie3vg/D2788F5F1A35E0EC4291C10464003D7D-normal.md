...
\[
{\mathbf{g}}^{d} = \operatorname{softmax}\left( {{\mathbf{W}}_{g}^{d}{\mathbf{x}}^{d}}\right) , \tag{4}
\]
...
To measure the distance between domains and consider the learning ability of the model, an intuitive approach is to compute the similarity between domain-specific representations, i.e., calculating \( \operatorname{dis}\left( {{\mathbf{h}}^{d},{\mathbf{h}}^{{d}^{\prime }}}\right) \) , where \( {\mathbf{h}}^{d} \) is the learned representation in \( d \) -th domain. However, such an approach would incur a huge time overhead of \( O\left( {N}^{2}\right) \) , where \( N \) is the number of samples. Even in the case of batch processing, the time complexity is still \( O\left( {B}^{2}\right) \) , where \( B \) is the batch size. This is unacceptable for the latency-sensitive recommendation field. In addition, unselectively measuring all data will inevitably be interfered with by low-quality or noisy data.
...
\[
{\mathcal{L}}_{rec} = \mathop{\sum }\limits_{{d = 1}}^{D}{\begin{Vmatrix}{\mathbf{h}}^{d} - {\widehat{\mathbf{h}}}^{d}\end{Vmatrix}}_{2}^{2} \tag{8}
\]
...
Output: Similar Domain Set \( {\left\{  {\mathcal{S}}^{d}\right\}  }_{d = 1}^{D} \) , Prediction Click Label \( \widehat{y} \)
		...
		while not converged do
			...
			Training model with training data \( \mathcal{T} \) and learn prototype \( p \)
...
REMARK 2. With the calculated domain distance, the combination of similar domains we need to search is drastically reduced from \( O\left( {2}^{D - 1}\right) \) to \( O\left( D\right) \) , where \( D \) is the number of domains. Taking the Movielens dataset as an example, if the distance from other domains to domain YOUNG is known to be sorted as YOUNG < MIDDLE < SENIOR, there are only three states that we need to discuss for similar domains of domain YOUNG, i.e., \( \left\lbrack  Y\right\rbrack  ,\left\lbrack  {Y, M}\right\rbrack \) , and \( \left\lbrack  {Y, M, S}\right\rbrack \) . The more the number of domains, the more the number of states is reduced.
...
\[
{\mathbf{h}}_{\text{ revised }}^{d} = {g}_{\text{ mask }}^{d}\left( {\mathbf{x}}^{d}\right) \mathbf{e}\left( {\mathbf{x}}^{d}\right) \tag{9}
\]

\[
{\mathbf{g}}_{\text{ mask }}^{d}\left( {x}^{d}\right)  = \operatorname{softmax}\left( {{\mathbf{g}}^{d} + {\mathcal{M}}^{d}}\right) \tag{10}
\]
...
\[
\mathop{\inf }\limits_{h}{\mathbb{E}}_{p}\left\lbrack  {{\ell }_{\mathrm{{CE}}}\left( {h\left( {\mathbf{Z}}^{i}\right) ,{\mathrm{Y}}_{i}}\right) }\right\rbrack   - \mathop{\inf }\limits_{{h}^{\prime }}{\mathbb{E}}_{p}\left\lbrack  {{\ell }_{\mathrm{{CE}}}\left( {{h}^{\prime }\left( {{\mathbf{X}}^{1},\ldots ,{\mathbf{X}}^{D}}\right) ,{\mathrm{Y}}_{i}}\right) }\right\rbrack   \geq  {\Delta }_{p}^{i},
\]
...
- Amazon-5core \( {}^{3} \) : The dataset is a compact subset from Amazon, including users and items with a minimum of 5 interactions. Three domains with overlapped users and items, "Clothing", "Beauty", and "Health" are utilized for the training and evaluation process. For simplicity, we denote this dataset as "Amazon".
...
<table><tr><td rowspan="2">AUC (↑)</td><td colspan="4">Movielens</td><td colspan="4">Amazon</td><td colspan="4">Douban</td></tr><tr><td>D1</td><td>D2</td><td>D3</td><td>Overall</td><td>D1</td><td>D2</td><td>D3</td><td>Overall</td><td>D1</td><td>D2</td><td>D3</td><td>Overall</td></tr><tr><td>MMOE <br> MMOE+SDSP</td><td>0.8114 <br> \( {\mathbf{{0.8197}}}^{ * } \)</td><td>0.8122 <br> 0.8191*</td><td>0.7998 <br> \( \mathbf{{0.8072}^{ * }} \)</td><td>0.8077 <br> \( \mathbf{{0.8151}^{ * }} \)</td><td>0.7005 <br> \( \mathbf{{0.7056}^{ * }} \)</td><td>0.6826 <br> 0.6841*</td><td>0.7323 <br> \( \mathbf{{0.7351}^{ * }} \)</td><td>0.7073 <br> 0.7107*</td><td>0.7082 <br> 0.7168*</td><td>0.7250 <br> 0.7308*</td><td>0.8109 <br> 0.8155*</td><td>0.7974 <br> \( \mathbf{{0.8024}^{ * }} \)</td></tr><tr><td>PLE <br> PLE+SDSP</td><td>0.8105 <br> \( {\mathbf{{0.8147}}}^{ * } \)</td><td>0.8134 <br> 0.8190*</td><td>0.8014 <br> \( \mathbf{{0.8090}^{ * }} \)</td><td>0.8085 <br> \( \mathbf{{0.8147}^{ * }} \)</td><td>0.7025 <br> 0.7049*</td><td>0.6796 <br> \( \mathbf{{0.6848}^{ * }} \)</td><td>0.7324 <br> \( \mathbf{{0.7342}^{ * }} \)</td><td>0.7075 <br> 0.7116*</td><td>0.7096 <br> 0.7168*</td><td>0.7305 <br> 0.7322*</td><td>0.8104 <br> 0.8165*</td><td>0.7976 <br> \( \mathbf{{0.8031}^{ * }} \)</td></tr><tr><td>SARNet <br> SARNet+SDSP</td><td>0.8123 <br> \( {\mathbf{{0.8130}}}^{ * } \)</td><td>0.8134 <br> 0.8159*</td><td>0.8004 <br> \( \mathbf{{0.8048}^{ * }} \)</td><td>0.8087 <br> \( \mathbf{{0.8113}^{ * }} \)</td><td>0.6978 <br> 0.6947</td><td>0.6785 <br> \( \mathbf{{0.6822}^{ * }} \)</td><td>0.7239 <br> 0.7261*</td><td>0.7020 <br> 0.7047*</td><td>0.7163 <br> 0.7205*</td><td>0.7345 <br> 0.7418*</td><td>0.8117 <br> 0.8137*</td><td>0.7991 <br> \( \mathbf{{0.8021}^{ * }} \)</td></tr></table>
...
![7_208_662_614_379_0.jpg](images/7_208_662_614_379_0.jpg)
...
[17] Xiangnan He, Kuan Deng, Xiang Wang, Yan Li, Yongdong Zhang, and Meng Wang. 2020. Lightgen: Simplifying and powering graph convolution network for recommendation. In Proceedings of the 43rd International ACM SIGIR conference on research and development in Information Retrieval. 639-648.
...
Lemma 4. [18] For any domain subset \( \mathbf{D} \subseteq  \mathcal{D} \) , we have

\[
H\left( {\mathrm{Y} \mid  {\mathrm{X}}^{\mathrm{D}}}\right)  = \mathop{\inf }\limits_{f}{\mathbb{E}}_{p}\left\lbrack  {{\ell }_{\mathrm{{CE}}}\left( {f\left( {\mathrm{X}}^{\mathrm{D}}\right) ,\mathrm{Y}}\right) }\right\rbrack
\]

Proof. Lemma1

\[
{\mathbb{E}}_{p}\left\lbrack  {{\ell }_{\mathrm{{CE}}}\left( {f\left( {\mathrm{X}}^{\mathrm{D}}\right) ,\mathrm{Y}}\right) }\right\rbrack
\]

\[
=  - {\mathbb{E}}_{p}\left\lbrack  {\mathbb{I}\left( {\mathrm{Y} = 0}\right) \log \left( {1 - f\left( {\mathrm{X}}^{\mathrm{D}}\right) }\right)  + \mathbb{I}\left( {\mathrm{Y} = 1}\right) \log \left( {f\left( {\mathrm{X}}^{\mathrm{D}}\right) }\right) }\right\rbrack
\]

\[
=  - {\mathbb{E}}_{D}{\mathbb{E}}_{Y}\left\lbrack  {\mathbb{I}\left( {\mathrm{Y} = 0}\right) \log \left( {1 - f\left( {\mathrm{X}}^{\mathrm{D}}\right) }\right)  + \mathbb{I}\left( {\mathrm{Y} = 1}\right) \log \left( {f\left( {\mathrm{X}}^{\mathrm{D}}\right) }\right)  \mid  {\mathrm{X}}^{\mathrm{D}}}\right\rbrack
\]

\[
=  - {\mathbb{E}}_{D}\left\lbrack  {\Pr \left( {\mathrm{Y} = 0 \mid  {\mathrm{X}}^{\mathrm{D}}}\right) \log \left( {1 - f\left( {\mathrm{X}}^{\mathrm{D}}\right) }\right)  + \Pr \left( {\mathrm{Y} = 1 \mid  {\mathrm{X}}^{\mathrm{D}}}\right) \log \left( {f\left( {\mathrm{X}}^{\mathrm{D}}\right) }\right) }\right\rbrack
\]
...
\[
\log \left( {1 - f\left( {\mathbf{X}}^{\mathbf{D}}\right) }\right)  + \Pr \left( {\mathbf{Y} = 1 \mid  {\mathbf{X}}^{\mathbf{D}}}\right) \log \frac{\Pr \left( {\mathbf{Y} = 1 \mid  {\mathbf{X}}^{\mathbf{D}}}\right) }{\Pr \left( {\mathbf{Y} = 1 \mid  {\mathbf{X}}^{\mathbf{D}}}\right) }
\]
...
\[
\log \frac{\Pr \left( {\mathrm{Y} = 1 \mid  {\mathbf{X}}^{\mathrm{D}}}\right) }{f\left( {\mathbf{X}}^{\mathrm{D}}\right) } -  - \Pr \left( {\mathrm{Y} = 0 \mid  {\mathbf{X}}^{\mathrm{D}}}\right) \log \left( {\Pr \left( {\mathrm{Y} = 0 \mid  {\mathbf{X}}^{\mathrm{D}}}\right) }\right)
\]
...
\[
\geq  H\left( {\mathbf{Y} \mid  {\mathbf{X}}^{\mathbf{D}}}\right) \text{ . }
\]

Table 5: The default parameter setting in our experiments

<table><tr><td>Dataset</td><td>Movielens</td><td>Amazon</td><td>Douban</td></tr><tr><td>Learning Rate</td><td>0.01</td><td>0.0001</td><td>0.001</td></tr><tr><td>Prototype Hyper-parameter \( \gamma \)</td><td>0.0001</td><td>0.0001</td><td>0.001</td></tr><tr><td>Prototype Number</td><td>10</td><td>10</td><td>10</td></tr><tr><td>Decay</td><td>0.9</td><td>0.9</td><td>0.9</td></tr><tr><td>Expert Num</td><td>\( \left\lbrack  {1,1,1}\right\rbrack \)</td><td>\( \left\lbrack  {1,1,1}\right\rbrack \)</td><td>\( \left\lbrack  {1,1,1}\right\rbrack \)</td></tr><tr><td>Batch Size</td><td>4096</td><td>4096</td><td>4096</td></tr></table>
...
where \( \widehat{f}\left( {{\mathbf{X}}^{\mathbf{D}},\mathrm{Y}}\right) \) is the prediction distribution for \( \mathrm{Y} \) . Specifically, \( \widehat{f}\left( \cdot \right) \) is defined as

\[
\widehat{f}\left( {{\mathbf{X}}^{\mathbf{D}},\mathrm{Y}}\right)  = \left\{  \begin{array}{ll} f\left( {\mathbf{X}}^{\mathbf{D}}\right) & \text{ when }\mathrm{Y} = 1 \\  1 - f\left( {\mathbf{X}}^{\mathbf{D}}\right) & \text{ when }\mathrm{Y} = 0 \end{array}\right.
\]

Considering \( {KL}\left( {\Pr \left( {\mathrm{Y} \mid  {\mathrm{X}}^{\mathrm{D}}}\right)  \geq  0}\right. \) , the infimum of Eq.(1) is easily to be acquire when the prediction distribution \( \left( {1 - f\left( {\mathrm{X}}^{\mathrm{D}}\right) , f\left( {\mathrm{X}}^{\mathrm{D}}\right) }\right) \) equals with the true label distribution \( \left( {\Pr \left( {\mathrm{Y} = 0 \mid  {\mathrm{X}}^{\mathrm{D}}}\right) ,\Pr (\mathrm{Y} = 1}\right. \) | \( {\mathrm{X}}^{\mathrm{D}} \) ).
...
\[
I\left( {{\mathbf{Z}}^{i};{\mathbf{Y}}^{i}}\right)  \leq  \mathop{\min }\limits_{{j \in  {\mathcal{S}}^{i}}}\left\{  {I\left( {{\mathbf{X}}^{j};{\mathrm{Y}}_{i}}\right) }\right\}   \leq  \mathop{\max }\limits_{{j \in  \mathcal{D}}}\left\{  {I\left( {{\mathbf{X}}^{j};{\mathrm{Y}}_{i}}\right) }\right\}   = I\left( {{\mathbf{X}}^{i};{\mathrm{Y}}^{i}}\right)
\]
...
where the last inequality follows from the fact that the joint mutual information \( I\left( {{\mathbf{X}}^{i};{\mathbf{Y}}^{i}}\right)  \leq  I\left( {{\mathbf{X}}^{1},\ldots ,{\mathbf{X}}^{D};{\mathbf{Y}}_{i}}\right) \) is at least as large as any one of \( I\left( {{\mathbf{X}}_{i}^{1};{\mathbf{Y}}_{i}}\right) \) and \( I\left( {{\mathbf{X}}_{i}^{2};{\mathbf{Y}}_{i}}\right) \) . Therefore, due to the variational form of the conditional entropy, we have

\[
\mathop{\inf }\limits_{h}{\mathbb{E}}_{p}\left\lbrack  {{\ell }_{\mathrm{{CE}}}\left( {h\left( {\mathbf{Z}}^{i}\right) ,{\mathbf{Y}}_{i}}\right) }\right\rbrack   - \mathop{\inf }\limits_{{h}^{\prime }}{\mathbb{E}}_{p}\left\lbrack  {{\ell }_{\mathrm{{CE}}}\left( {{h}^{\prime }\left( {{\mathbf{X}}^{1},\ldots ,{\mathbf{X}}^{D}}\right) ,{\mathbf{Y}}_{i}}\right) }\right\rbrack
\]
...
\[
= I\left( {{\mathbf{X}}^{i};{\mathbf{Y}}^{i}}\right)  - \mathop{\min }\limits_{{j \in  {\mathcal{S}}^{i}}}\left\{  {I\left( {{\mathbf{X}}^{j};{\mathbf{Y}}^{i}}\right) }\right\}   = {\Delta }_{p}^{i}
\]
...
As shown in Table 6, integrating the SDSP module into baseline models results in a minimal increase in parameter size. For instance, integrating SDSP into MMOE increases parameters by only \( {86.05}\mathrm{\;K} \) for Movielens, from 217.64K to 303.68K. Similarly, PLE+SDSP increases by \( {89.77}\mathrm{\;K} \) . These results demonstrate that our method is lightweight, adding minimal parameter cost while achieving improved performance, making it suitable for resource-constrained scenarios without significant computational burden.
